...
/*   Updated: 2015/12/11 12:49:29 by fdaudre-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							const t_matrix *p1, const t_matrix *p2)
{
	...
	if ((p.x < 0) || (p.y > datas->nlines))
		return ;
	e = (int)(p2->tab[0][0] - p1->tab[0][0]);
	xmax = FT_MIN(datas->lsize / datas->bpp * 8, (int)p2->tab[0][0] + 1);
	while (p.x < xmax)
	{
							...
							const t_matrix *p1, const t_matrix *p2)
{
	...
	if ((p.x < 0) || (p.x > comp))
		return ;
	e = (int)(p2->tab[0][1] - p1->tab[0][1]);
	ymax = FT_MIN(datas->nlines, (int)p2->tab[0][1]) + 1;
	while (p.y < ymax)
	{
		ft_put_pixel(datas, &p, fcolor(datas, p1, p2, &p));
							...
							const t_matrix *p1, const t_matrix *p2)
{
	...
	if ((p.y < 1) || (p.x < 0) || (p.x > comp))
		return ;
	e = (int)(p2->tab[0][1] - p1->tab[0][1]);
	ymin = FT_MAX(0, (int)p2->tab[0][1]) - 1;
	while (p.y > ymin)
	{
		ft_put_pixel(datas, &p, fcolor(datas, p1, p2, &p));
		if ((e += 2 * (p2->tab[0][0] - p1->tab[0][0])) > 0)
